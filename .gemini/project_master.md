...
- **3.4: QLoRA-обучение с помощью Kubeflow Training Operator**: Реализовать обучение, разделив его на две части согласно новой роле-ориентированной модели:
    - **`TrainingRuntime` (DevOps-задача)**: В Python-коде определить среду выполнения: базовый образ, зависимости (`unsloth`, `transformers`), и требования к ресурсам (запросить **все** виртуальные ресурсы GPU, например 4, чтобы монополизировать физическую карту).
...
- **5.4: «Боевой» деплой основной модели**: Развернуть нашу последнюю **основную, дообученную модель** `Production` из MLflow Registry через отдельный `InferenceService`. Этот сервис будет:
    1.  Запрашивать **все доступные виртуальные ресурсы GPU** (например, `nvidia.com/gpu: 4`), чтобы монополизировать всю карту для максимальной производительности.
