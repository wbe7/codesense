...
Для запуска процесса индексации кода и загрузки его в векторную базу данных Qdrant используется скрипт `run.py` в модуле `codesense.pipelines.indexing`.
...
python -m codesense.pipelines.indexing.run --collection-name "имя-вашей-коллекции"
```
...
*   `--collection-name` (обязательный): Имя коллекции в Qdrant, в которую будут сохраняться эмбеддинги.
    *   *Пример:* `--collection-name "codesense-dev"`

*   `--batch-size` (опциональный, по умолчанию: `64`): Количество чанков, которые обрабатываются и загружаются в Qdrant за один раз.
    *   *Рекомендация:* Уменьшите это значение (например, до `8` или `4`), если вы сталкиваетесь с ошибками нехватки видеопамяти (CUDA out of memory) на GPU.
